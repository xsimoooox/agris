...
if (startAnalysisBtn) {
    startAnalysisBtn.addEventListener('click', () => {
        ...
        let selectedCrop = selectedItem.getAttribute('data-crop');
        if (selectedCrop === 'other') {
            const customVal = selectedItem.getAttribute('data-custom-name') || document.getElementById('custom-crop-input')?.value.trim();
            if (customVal) {
        ...
        // Save selected crop to local storage for persistence across dashboard
        ...
        setTimeout(() => {
            ...
            window.location.href = `../gaia_dashboard/${targetFile}?crop=${encodeURIComponent(selectedCrop)}&area=${areaHa}&mode=${mode}${dateQuery}`;
        }, 2000);
    });
}
...
const customCropContainer = document.getElementById('custom-crop-input-container');
const customCropInput = document.getElementById('custom-crop-input');
const customCropSubmit = document.getElementById('custom-crop-submit-btn');
const otherCropLabel = document.getElementById('other-crop-label');

function selectCropItem(item) {
    const isOther = item.getAttribute('data-crop') === 'other';

    if (isOther) {
        // Show custom crop input box
        if (customCropContainer) {
            customCropContainer.classList.remove('hidden');
            if (customCropInput) {
                customCropInput.focus();
                customCropContainer.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }
        }

        const currentCustomName = item.getAttribute('data-custom-name') || customCropInput?.value.trim();
        if (currentCustomName) {
            cropItems.forEach(i => i.classList.remove('selected'));
            item.classList.add('selected');
            if (startAnalysisBtn) startAnalysisBtn.classList.remove('hidden');
        } else {
            // Keep existing selection or highlight box until user enters name
            cropItems.forEach(i => i.classList.remove('selected'));
            item.classList.add('selected');
        }
    } else {
        // Hide custom crop input container if open
        if (customCropContainer) {
        ...
        if (startAnalysisBtn) {
            ...
            startAnalysisBtn.scrollIntoView({ behavior: 'smooth', block: 'end' });
        }
    }
}

cropItems.forEach(item => {
    item.addEventListener('click', () => selectCropItem(item));
...
function confirmCustomCrop() {
    if (!customCropInput) return;
    const val = customCropInput.value.trim();
    if (!val) {
        customCropInput.focus();
        return;
    }

    const otherCard = document.querySelector('.crop-item[data-crop="other"]');
    if (otherCard) {
        otherCard.setAttribute('data-custom-name', val);
        cropItems.forEach(i => i.classList.remove('selected'));
        otherCard.classList.add('selected');
        if (otherCropLabel) {
            otherCropLabel.textContent = val;
        }
    }
...
if (customCropSubmit) {
    customCropSubmit.addEventListener('click', confirmCustomCrop);
}

if (customCropInput) {
    customCropInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            confirmCustomCrop();
        }
...
document.querySelectorAll('.tag-example-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        if (customCropInput) {
            customCropInput.value = btn.textContent.trim();
            confirmCustomCrop();
        }
    });
});
