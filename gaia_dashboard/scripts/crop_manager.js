...
function updatePageTerminology() {
    ...
    const lang = (typeof getSavedLang === 'function') ? getSavedLang() : (localStorage.getItem('agrisense_lang') || localStorage.getItem('agriscience_lang') || localStorage.getItem('terranova_lang') || 'fr');
    const isAr = lang === 'ar';

    // 1. BRANDING: Replace residual TerraNova / AgriScience occurrences in visible UI
    ...
    allTextElements.forEach(el => {
        ...
        if (el.children.length === 0) {
            if (/TerraNova|Terranova|TERRANOVA|AgriScience|AGRISCIENCE/i.test(el.textContent)) {
                el.textContent = el.textContent
                    .replace(/TerraNova|Terranova|AgriScience/g, 'Agrisense')
    ...
    if (!cropBadge && mainTitle) {
        ...
        cropBadge.setAttribute('style', `
            ...
            font-weight: 600;
            color: #072167;
            box-shadow: 0 4px 15px rgba(7, 33, 103, 0.05);
    ...
    if (/TERRANOVA|Terranova|AGRISCIENCE|AgriScience/i.test(document.title)) {
